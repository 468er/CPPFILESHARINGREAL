...
string Scene::toString(){
	...
	oss << "Act: " << actNumber << " Scene: " << sceneNumber << " Location: " << location << endl;
...
//			std::transform(currentLine.begin(), currentLine.end(), currentLine.begin(),[](unsigned char c){ return std::tolower(c); });
			...
			bool misMatch = false;
        	    if (currentLine[b] == word[0] || std::tolower(currentLine[b]) == std::tolower(word[0])) {
			//try using substring instead of all the code below
			string testStr = currentLine.substr(b, word.length());
			for(int d = 0; d < testStr.length(); d++){
				if(/*std::tolower(testStr[d]) != std::tolower(word[d]) ||*/ word[d] != testStr[d]){
					misMatch = true;
				}
			}
			if(misMatch){
